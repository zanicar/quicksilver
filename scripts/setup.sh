...
if [ "$IS_MULTI_ZONE_TEST" = true ]; then
    ...
    sed -i -e "s/seeds = \".*\"/seeds = \"\"/g" ${CHAIN_DIR}/${CHAINID_2}/config/config.toml
    ...
    sed -i -e "s/seeds = \".*\"/seeds = \"\"/g" ${CHAIN_DIR}/${CHAINID_2}a/config/config.toml
    ...
    sed -i -e "s/seeds = \".*\"/seeds = \"\"/g" ${CHAIN_DIR}/${CHAINID_2}b/config/config.toml
    ...
    sed -i -e "s/seeds = \".*\"/seeds = \"\"/g" ${CHAIN_DIR}/${CHAINID_2}c/config/config.toml
...
jq '.app_state.interchainstaking.params.delegation_account_count = 12' ./${CHAIN_DIR}/${CHAINID_0}/config/genesis.json > ./${CHAIN_DIR}/${CHAINID_0}/config/genesis.json.new && mv ./${CHAIN_DIR}/${CHAINID_0}/config/genesis.json{.new,}
